...
const initialBookingAdvancePayment = async (
  ...
  const res = await axios.post(config.PAYMENT_URL!, {
    ...
    success_url: `http://localhost:5001/api/payment/bookingConfirmation?transactionid=${paymentDetails?.transactionId}&status=success`,
    fail_url: `http://localhost:5001/api/payment/bookingConfirmation?transactionid=${paymentDetails?.transactionId}&status=failed`,
