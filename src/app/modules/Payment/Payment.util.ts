...
const initialBookingAdvancePayment = async (
  ...
  const res = await axios.post(config.PAYMENT_URL!, {
    ...
    cancel_url: `https://bond-bike-rental-frontend.vercel.app/allbikes`,
...
const rentalPayment = async (paymentDetails: TRentalpaymentDetails) => {
  const res = await axios.post(config.PAYMENT_URL!, {
    ...
    cancel_url: `https://bond-bike-rental-frontend.vercel.app/allbikes`,
    // success_url: `http://localhost:5000/api/payment/rentalPayment?transactionid=${paymentDetails?.transactionId}&status=success&user=${paymentDetails.user._id}&booking=${paymentDetails.bookingId}&amount=${paymentDetails.amount}`,
    // fail_url: `http://localhost:5000/api/payment/rentalPayment?status=failed`,
    // cancel_url: `http://localhost:5173/allbikes`,
    amount: paymentDetails.amount,
    currency: 'BDT',
